...
static VALUE
rbpcap_dump_open(VALUE self, VALUE filename, VALUE link)
{
	rbpcap_dump_t *rbpd;

    Data_Get_Struct(self, rbpcap_dump_t, rbpd);

	rbpd->pd = pcap_open_dead(NUM2INT(link), 1514);
	if(!rbpd->pd)
		rb_raise(rb_eRuntimeError, "pcap_open_dead failed");

	rbpd->dumper = pcap_dump_open(rbpd->pd, RSTRING(filename)->ptr);
	if(!rbpd->dumper)
		rb_raise(rb_eRuntimeError, "pcap_dump_open failed");

	return self;
}

static VALUE
rbpcap_dump(VALUE self, VALUE packet)
{
	rbpcap_dump_t *rbpd;
	...
	memset(&h, 0, sizeof(h));
	h.caplen = RSTRING(packet)->len;
	h.len = RSTRING(packet)->len;

	pcap_dump((unsigned char *) rbpd->dumper, &h, (unsigned char *) RSTRING(packet)->ptr);
...
static VALUE
rbpcap_dump_open_s(VALUE class, VALUE filename, VALUE link)
{
    VALUE iPcap = rb_funcall(rb_cPcap, rb_intern("new"), 0);

    return rbpcap_dump_open(iPcap, filename, link);
}
...
Init_pcaprub()
{
    ...
    rb_define_singleton_method(rb_cPcap, "dump_open", rbpcap_dump_open_s, 2);

    rb_define_method(rb_cPcap, "each", rbpcap_capture, 0);
    rb_define_method(rb_cPcap, "dump", rbpcap_dump, 1);
